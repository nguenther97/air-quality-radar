...
async function fetchSource(name, url, parse, cache) {
  try {
    ...
    return { readings, status: { source: name, ok: true }, raw };
  } catch (err) {
...
async function main() {
  ...
  for (const f of airnow.raw ? parseAirNowForecast(airnow.raw) : []) {
    const existing = forecastById.get(f.id);
    if (!existing || existing.dayOffset > f.dayOffset) forecastById.set(f.id, f);
  }
  for (const f of canadaForecastRaw ? parseCanadaAQHIForecast(JSON.parse(canadaForecastRaw), now) : []) {
